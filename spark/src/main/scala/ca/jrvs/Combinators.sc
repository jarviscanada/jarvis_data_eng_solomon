...
val pairedList = names.zip(countryMap.values)
 ...
 */
//write you solution here
names.map(countryMap.getOrElse(_, "n/a")).groupBy(identity).mapValues(_.size)
 ...
 */
//write you solution here
val upperCity = employeeList.map(_.name.toUppercase)
